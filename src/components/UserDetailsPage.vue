  ...
  <Card>
    ...
    <CardContent>
      ...
      <form class="grid gap-4 py-4" v-else>
        ...
        <!-- Password -->
        <div class="grid grid-cols-4 items-center gap-4">
          <Label for="password" class="text-right"> Password (leave blank to keep the same) </Label>
          <Input id="password" v-model="password" type="password" class="col-span-3" />
        </div>
      </form>
    </CardContent>
    <CardFooter class="pt-4">
      <Button @click="startEditing" v-if="!isEditing" class="mb-4"> Change Details </Button>
      <div class="flex justify-end" v-if="isEditing">
        <Button @click="cancelEditing" class="mr-4"> Cancel </Button>
        <Button type="submit" @click="validateForm"> Save Changes </Button>
      </div>
    </CardFooter>
  </Card>
</template>